...
if __name__ == "__main__":
    onnx_model = onnx.load_model("./flan-t5-small-onnx/decoder_model.onnx")
    model = ModelFactory().open(model=onnx_model)
    model_json = model.to_json()
    i_dict = {value["name"]: i for i, value in enumerate(model_json["graphs"][0]["arguments"])}
    print("inspect now")
